...
<script type="text/javascript">
	$(function() {
		...
		$("#deleteInformation").click(function(){
			...
			showCallBackDialog("确定删除?",function(){
				$.post("${ctx}/admin/information/help/delInformations",{ids:ids},function(data){
					if(data.code==0){
		...
		$("#addInformation").click(function(){
			location.href="${ctx}/admin/information/help/toAddPage";
		})
	...
	function delInformation(id){
		showCallBackDialog("确定删除?",function(){
			$.post("${ctx}/admin/information/activity/delInformation",{id:id},function(data){
				if(data.code==0){
					showCallBackDialog("删除成功",function(){
						location.href = "${ctx}/admin/information/help/list";
					})
	...
	function closeEdit(id){
		...
		$.post("${ctx}/admin/information/help/changeSortid",{id:id,sortid:sortid},function(data){
			if(data.code==0){
				showCallBackDialog("修改成功",function(){
					location.href = "${ctx}/admin/information/help/list?page=${pageNumber}&sortType=${sortType}";
				})
			}else{
				alert(data.mes);
			}
...
<div class="row  border-bottom">
	<div class="basic">
        <p>文章管理</p>
        <span><a href="<c:url value='/admin/main'/>"  style="margin-left:0;">首页</a>><a href="#" >文章管理</a>><a><strong>帮助中心</strong></a></span>
    </div>
</div>    
		<form id="myform" action="${ctx }/admin/information/help/list" >
		...
		<div class="employee animated fadeInRight">
	           <dl>
	           	<dt style="width:0;"></dt>
	             <dd>
	               <a href="#" > <samp id="addInformation" >添加帮助</samp> </a>
            ...
            <table width="100%" class="table table-striped" border="0" align="center" cellpadding="0" cellspacing="0">
                 ...
                 <tbody>
                     <c:forEach var="information" items="${page.getContent() }" >
                       <tr >
	                     	<td class="l_in2"><input name="information" value="${information.id }" type="checkbox" class="l_in childcheckbox"></td>
	                        ...
	                        <c:if test="${information.status eq 'NO'}" >
	                       	...
	                       	<%-- <td><font>${information.sortid }</font><img src="${ctx }/static/brd/img/sr.png"></td> --%>
	                        <td> 
	                        ...
	                        <td>
	                        	<a href="${ctx}/admin/information/help/toEditInformation/${information.id}?sortType=${sortType}&page=${page.getNumber() + 1}"  class="l_a"><img src="${ctx }/static/brd/img/bjt1.png"></a>
	                        	<a href="#"  class="l_a"><img onclick="delInformation('${information.id}')" src="${ctx }/static/brd/img/bjt3.png"></a>
	                            <a href="${ctx }/pc/content/${information.id}" target="_blank"  class="l_a"><img src="${ctx }/static/brd/img/bjt2.png"></a>
	                        </td>
                     	</tr>  
                     </c:forEach>
                   </tbody>
							...
							<div class="btn-group">
								<div class="dataTables_paginate paging_simple_numbers"
									...
									<tags:pagination page="${page}" paginationSize="10"
										...
										hrefPrefix="${ctx}/admin/information/help/list" />
								</div>
							</div>
						</div>
            </div>
