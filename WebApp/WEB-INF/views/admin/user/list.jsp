...
<body>
	...
	<div class="row border-bottom">
		<div class="basic">
	   		<p>司机管理</p>
	        <span><a href="${ctx }/admin/main;JSESSIONID=<%=request.getSession().getId()%>"  style="margin-left:0;">首页</a>><a href="#" >用户管理</a>><a><strong>用户列表</strong></a></span>
	    </div>
    </div>
    <div class="employee animated fadeInRight ">
       ...
       <dl>
       	 <dt>操作：</dt>
         <dd>
			 <a href="${ctx}/admin/user/toAddUser" ><samp>新增用户</samp></a>
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class ="table table-striped">
             <tbody>
                 ...
                 <c:forEach items = "${users.content }" var ="users">
                 	<tr>
                 		<td></td>
	                    <td>${users.username }</td>
						<td>${users.realname }</td>
						<td>${users.mobileno }</td>
						<td class="l_ls">${users.state.getDes() }</td>
						<td>${util:formatNormalDate(users.createTime) }</td>
	                    <td><a href="${ctx }/admin/user/toEditDriveUser/${users.id}"><span><img src="${ctx }/static/brd/img/bjt1.png"></span></a></td>
                 	</tr>
                 </c:forEach>
       			...
       			<div class=" m_right">
                     <tags:pagination paginationSize="10" page="${driverforms}" hrefPrefix="${ctx }/admin/user/list" hrefSubfix="${queryStr}"></tags:pagination>
                 </div>
        </div>
        </div>
        </div>
