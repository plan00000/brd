...
<script type="text/javascript">
	$(function() {
		...
		if(rolename=='Admin'|| rolename=='业务员' || rolename=='风控经理' || rolename=='财务'){
			$("#rolename").attr("disabled",true);
		}
		
		var initpermissions = '${role.permissions}';
		var permissionsarray=initpermissions.split(";");
		for(var index in permissionsarray){
			$('input[name="permissions"]').each(function(){
				permission= $(this).val();
				if(permission===permissionsarray[index]){
					$(this).iCheck("check");					
				}				
			});
		}
		...
		$("#submit").click(function(){
			...
			$.post("${ctx}/admin/role/editRole",dto,function(data){
				if(data.code==0){
					showCallBackDialog(data.mes,function(){
						location.href = "${ctx}/admin/role/list?page=${page}";
					})
				}else{
					alert(data.mes);	
				}				
...
<div class="row  border-bottom">
	<div class="basic">
        <p>企业管理</p>
        <span><a href="<c:url value='/admin/main'/>"  style="margin-left:0;">首页</a>><a href="#" >企业管理</a>><a href="#" >角色管理</a>><a><strong>编辑角色</strong></a></span>
    </div>
	</div>
					...
					<div class="ibox-content">
						<div class="character">
			            	<div class="character01">
			                    ...
			                    <dl>
			                        <dt>角色名称：</dt>
			                        <dd><input type="text" value="${util:htmlEscape(role.rolename)}" name="rolename" id="rolename" maxlength="8" class="c_right"
			                         onKeyDown="gbcount(this.form.rolename,this.form.used,8);"  onKeyUp="gbcount(this.form.rolename,this.form.used,8);" >
		                   					<input style="width:25px;border:none;background:#fff;" id="valueDepartment" name="used" value="${fn:length(role.rolename) }/8" readonly="readonly" /> 
			                        </dd>
			                 ...
			                 <div class="character01 hidden">
			                    ...
			                    <dl>
			                        ...
			                        <dd class="c_ter"><input type="checkbox" name="permissions" id="login_admin" value="LOGIN_MANAGER_BACK"  class="c_box">登录后台</dd>
			                    </dl>
			                 </div>
			                    ...
			                    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
			                        <tbody>
			                            <tr>
			                              ...
			                              <tr bgcolor="f3f3f4">
			                            	<td class="c_tr"><input type="checkbox" <c:if test="${fn:contains(role.permissions, 'ORGANIZATION_MANAGER_DEPART') && fn:contains(role.permissions, 'ORGANIZATION_MANAGER_ROLE') && fn:contains(role.permissions, 'ORGANIZATION_MANAGER_USER')  }" >checked</c:if>  name="ORGANIZATION_MANAGER" class="c_tr1"></td>
			                            ...
			                            <tr>
			                            	<td class="c_tr"><input type="checkbox" <c:if test="${fn:contains(role.permissions, 'USER_MANAGER') && fn:contains(role.permissions, 'USER_UPGRADE') }" >checked</c:if>  name="USER" class="c_tr1"></td>
			                             ...
			                             <tr bgcolor="f3f3f4">
			                            	<td class="c_tr"><input type="checkbox" <c:if test="${fn:contains(role.permissions, 'ORDER_MANAGER') && fn:contains(role.permissions, 'ORDER_MANAGER_PC') && fn:contains(role.permissions, 'ORDER_MANAGER_BROKERAGE') && fn:contains(role.permissions, 'ORDER_MANAGER_WITHDRAW')  }" >checked</c:if>   name="ORDER_MANAGER" class="c_tr1"></td>
			                             ...
			                             <tr>
			                            	<td class="c_tr"><input type="checkbox" <c:if test="${fn:contains(role.permissions, 'PRODUCT_MANAGER_TYPE') && fn:contains(role.permissions, 'PRODUCT_MANAGER') && fn:contains(role.permissions, 'ORGANIZATION_MANAGER_USER')  }" >checked</c:if> name="PRODUCT_MANAGER" class="c_tr1"></td>
			                             ...
			                             <tr bgcolor="f3f3f4">
											 <td class="c_tr"><input type="checkbox" <c:if test="${fn:contains(role.permissions, 'ACTIVITY_MANAGER_STAR') && fn:contains(role.permissions, 'ACTIVITY_MANAGER_APPRENTICE') && fn:contains(role.permissions, 'ACTIVITY_MANAGER_REGISTER') && fn:contains(role.permissions, 'ACTIVITY_MANAGER_QRCODE')}" >checked</c:if>  name="ACTIVITY_MANAGER" class="c_tr1"></td>
			                             ...
			                             <tr>
			                             	<td class="c_tr" ><input type="checkbox" <c:if test="${fn:contains(role.permissions,'TEXT_MANAGER_ACTIVITY') && fn:contains(role.permissions,'TEXT_MANAGER_HELP') }" >checked</c:if>  name="TEXT_MANGER" class="c_tr1" ></td>
			                             ...
			                             <tr bgcolor="f3f3f4">
			                             	<td class="c_tr" ><input type="checkbox" <c:if test="${fn:contains(role.permissions,'TEXT_MANGER_PC_ACTIVITY') && fn:contains(role.permissions,'TEXT_MANAGER_APPRENTICE') && fn:contains(role.permissions,'TEXT_MANAGER_ABOUTUS') }" >checked</c:if> name="TEXT_MANGER_PC" class="c_tr1" ></td>
			                            ...
			                            <%--  <tr>
			                            	<td class="c_tr"><input type="checkbox"  <c:if test="${fn:contains(role.permissions, 'TEXT_MANAGER_ACTIVITY') && fn:contains(role.permissions, 'TEXT_MANAGER_HELP') && fn:contains(role.permissions, 'TEXT_MANAGER_APPRENTICE') && fn:contains(role.permissions, 'TEXT_MANAGER_ABOUTUS')  }" >checked</c:if>  name="TEXT_MANAGER" class="c_tr1"></td>
			                             ...
			                             <tr >
			                            	<td class="c_tr"><input type="checkbox" <c:if test="${fn:contains(role.permissions, 'AD_MANAGER_PC') && fn:contains(role.permissions, 'AD_MANAGER_WEIXIN') && fn:contains(role.permissions, 'ORGANIZATION_MANAGER_USER')  }" >checked</c:if>  name="AD_MANAGER" class="c_tr1"></td>
			                             ...
			                             <tr bgcolor="f3f3f4">
			                            	<td class="c_tr"><input type="checkbox" <c:if test="${fn:contains(role.permissions, 'SET_FRIENDSHIPLINK')  && fn:contains(role.permissions, 'SET_MANAGER_BASE') && fn:contains(role.permissions, 'SET_MANAGER_INFO') && fn:contains(role.permissions, 'SET_MANAGER_AGGREMENT') && fn:contains(role.permissions, 'SET_SEO_SETTING') }" >checked</c:if>  name="SET_MANAGER" class="c_tr1"></td>
			                             ...
			                             <tr >
			                            	<td class="c_tr"><input type="checkbox" <c:if test="${fn:contains(role.permissions, 'STATISTICS_MANAGER_USER') && fn:contains(role.permissions, 'STATISTICS_REPORT_USER') && fn:contains(role.permissions, 'STATISTICS_MANAGER_ORDER') && fn:contains(role.permissions, 'STATISTICS_MANAGER_PRODUCT')&& fn:contains(role.permissions, 'STATISTICS_MANAGER_DEPART') && fn:contains(role.permissions, 'STATISTICS_MANAGER_BROKERAGE')  }" >checked</c:if>  name="STATISTICS_MANAGER" class="c_tr1"></td>
			                 ...
			                 <div class="submit" align="center">
			                     <div class="submit_trr">
			                        ...
			                        <a href="${ctx}/admin/role/list?page=${page}" id="backPage" ><p>返回</p></a>
			                     </div>
			                 </div>
							</div>	
			            </div>
