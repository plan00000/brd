...
$(function(){
	...
	$("#btn_save").click(function(){
		if(check()){
			$.post("${ctx}/admin/product/addProduct;JSESSIONID=<%=request.getSession().getId()%>",$("#submit_form").serialize(),
				function(data){
					if(data.code == 1){
	...
	$("#btn_back").click(function(){
		location.href = "${ctx}/admin/product/toChooseType;JSESSIONID=<%=request.getSession().getId()%>";
	})
})
function check(){
	/** 提单类型*/
	var billType = ${productType.billType.ordinal() };
...
function checkRateDesc(){
	...
	var userRateDesc = $("#ipt_userratedesc").val().trim();
...
function checkExpense(){
	...
	if(interestType =='1'){
		//检查小数点后是否对于两位
	    if (expense.split(".").length > 1 && expense.split(".")[1].length > 2) {
			alert("日息小数点不超过二位");
	        return false;
	    }else
	    if (expense.split(".").length > 1 && expense.split(".")[0].length > 2) {
	    ...
	    if (expense.split(".").length < 2 && expense.length > 3) {
...
function checkPercentageRate(){
	...
	/* if(interestType =='1'){
	    ...
	    if (percentageRate.split(".").length > 1 && percentageRate.split(".")[0].length > 3) {
	        alert("提成比例不能超过1000%");
	        return false;
	    }else
	    if (percentageRate.split(".").length < 2 && percentageRate.length > 3) {
	        alert("提成比例不能超过1000%");
	        return false;
	    }
	} */
...
<body>
	...
	<div class="new_emp animated fadeInRight">
		...
		<input type = "hidden" name = "mortgageType" value = "${mortgageType }"/>
		<input type = "hidden" name = "interestType" value = "${interestType }"/>
		<%-- <c:if test = "${productType.billType != 'SELFHELPLOAN' }"> --%>
        <div class="new_xinxi"><p><font>佣金计算参数 </font></p></div>
          <div class="compute">
          <!--  赚差价-按月-->
          	<c:if test = "${interestType eq '0' && productType.billType =='EARNDIFFERENCE'}">
              <dl>
                  <dt>月息底价：</dt>
                  <dd>
                  <input type="text" class="c_scet" id = "ipt_spread" name = "spread"><font>%/月</font>
                 </dd>
              </dl>
              <dl>
                  <dt>加价息差：</dt>
                  <dd>
                      <input type="text" class="c_scet c_cpt" id ="ipt_spreadMin" name = "spreadMin">
                      <font style="width:30px;">—</font>
                      <input type="text" class="c_scet c_cpt" id = "ipt_spreadMax" name = "spreadMax">
                      <font>%/月</font>
                 </dd>
              </dl>
            </c:if>
            <!-- 赚差价-按日 -->
            <c:if test = "${interestType eq '1' && productType.billType =='EARNDIFFERENCE'}">
              <dl>
                  <dt>日息底价：</dt>
                  <dd>
                  <input type="text" class="c_scet" id = "ipt_spread" name = "spread"><font>‰/日</font>
                 </dd>
              </dl>
              <dl>
                  <dt>加价息差：</dt>
            ...
            <!-- 赚差价-收益金 -->
            <c:if test = "${interestType eq '2' && productType.billType =='EARNDIFFERENCE'}">
              <dl>
                  <dt>收益金底价：</dt>
                  <dd>
                  <input type="text" class="c_scet" id = "ipt_spread" name = "spread"><font>%</font>
                 </dd>
              </dl>
              <dl>
                  <dt>约定费用价差：</dt>
            ...
            <!-- 赚提成-按月 -->
            <c:if test = "${interestType eq '0' && (productType.billType =='EARNCOMMISSION' || productType.billType == 'SELFHELPLOAN')}">
              <dl>
                  <dt>月息：</dt>
                  <dd>
                  <input type="text" class="c_scet" id = "ipt_expense" name = "expense"><font>%/月</font>
                 </dd>
              </dl>
              <c:if test = "${productType.billType != 'SELFHELPLOAN' }">
	              <dl>
	                  <dt>提成比例：</dt>
	                  <dd>
	                  <input type="text" class="c_scet" id = "ipt_percentageRate" name = "percentageRate"><font>%</font>
	                 </dd>
	              </dl>
              </c:if>
            </c:if>
            <!-- 赚提成-按日 -->
            <c:if test = "${interestType eq '1' && (productType.billType =='EARNCOMMISSION'|| productType.billType == 'SELFHELPLOAN')}">
              <dl>
                  <dt>日息：</dt>
                  <dd>
                  <input type="text" class="c_scet" id = "ipt_expense" name = "expense"><font>‰/日</font>
                 </dd>
              </dl>
              <c:if test = "${productType.billType != 'SELFHELPLOAN' }">
	              <dl>
            ...
            <!-- 赚提成-收益金 -->
            <c:if test = "${interestType eq '2' && (productType.billType =='EARNCOMMISSION'|| productType.billType == 'SELFHELPLOAN')}">
              <dl>
                  <dt>手续费用：</dt>
                  <dd>
                  <input type="text" class="c_scet" id = "ipt_expense" name = "expense"><font>%</font>
                 </dd>
              </dl>
              <c:if test = "${productType.billType != 'SELFHELPLOAN' }">
	              <dl>
            ...
            <c:if test = "${productType.billType.ordinal() != '0' && interestType !='3'}">
	           <dl>
		            <dt>前端佣金比例文字：</dt>
		            <dd>
		            <input type="text" class="c_scet" id ="ipt_fontBrokerageDesc" name = "fontBrokerageDesc"
		            ...
		            <input type="text" class="c_scet" id = "ipt_fontBrokerageNum" name = "fontBrokerageNum"
         ...
         <div class="special">
            <table width="100%" cellpadding="0" cellspacing="0">
            	<tbody>
                    ...
                    <tr>
                        <td>6个月</td>
                        <td><input type="text" class="s_cal ipt_algoParam" data-data-str="6个月收益金" id ="ipt_algoParamA" name = "algoParamA"></td>
                        <td><input type="text" class="s_cal ipt_algoParam" data-data-str="6个月佣金" id ="ipt_algoParamB" name = "algoParamB"></td>
                    </tr>
                    <tr>
                        <td>12个月</td>
                        <td><input type="text" class="s_cal ipt_algoParam" data-data-str="12个月收益金" id ="ipt_algoParamC" name = "algoParamC"></td>
                        <td><input type="text" class="s_cal ipt_algoParam" data-data-str="12个月佣金" id ="ipt_algoParamD" name = "algoParamD"></td>
                    </tr>
                    <tr>
                        <td>24个月</td>
                        <td><input type="text" class="s_cal ipt_algoParam" data-data-str="24个月收益金" id ="ipt_algoParamE" name = "algoParamE"></td>
                        <td><input type="text" class="s_cal ipt_algoParam" data-data-str="24个月佣金" id ="ipt_algoParamF" name = "algoParamF"></td>
                    </tr>
                    <tr>
                        <td>36个月</td>
                        <td><input type="text" class="s_cal ipt_algoParam" data-data-str="36个月收益金" id ="ipt_algoParamG" name = "algoParamG"></td>
                        <td><input type="text" class="s_cal ipt_algoParam" data-data-str="36个月佣金" id ="ipt_algoParamH" name = "algoParamH"></td>
                    </tr>
                </tbody>
            </table>
            <c:if test = "${productType.billType.ordinal() != '0' }">
	           <dl>
		            <dt>前端佣金比例文字：</dt>
		            <dd>
		            <input type="text" class="s_dt" id ="ipt_fontBrokerageDesc" name = "fontBrokerageDesc"
		             onKeyDown="gbcount(this.form.fontBrokerageDesc,this.form.used9,8);"  onKeyUp="gbcount(this.form.fontBrokerageDesc,this.form.used9,8);" onblur = "gbcount(this.form.fontBrokerageDesc,this.form.used9,8);">
		             <input class="inputtext2 inputtextn" name="used9"  id="tfontBrokerageDesc" value="0/8" readonly="readonly"> 
		            <samp>前端佣金数值：</samp>
		            <input type="text" class="s_dt" id = "ipt_fontBrokerageNum" name = "fontBrokerageNum"
		            onKeyDown="gbcount(this.form.fontBrokerageNum,this.form.used10,5);"  onKeyUp="gbcount(this.form.fontBrokerageNum,this.form.used10,5);" onblur = "gbcount(this.form.fontBrokerageNum,this.form.used10,5);">
		             <input class="inputtext2 inputtextn" name="used10"  id="tfontBrokerageNum" value="0/5" readonly="readonly"> 
		           </dd>
		        </dl>
          ...
          <div class="new_xinxi n_top"><p><font>佣金设置</font></p></div>
              <div class="compute">
                  ...
                  <dl>
                      <dt>师父佣金比例：</dt>
                      <dd>
                      <input type="text" class="c_scet" id = "ipt_fatherRate" name = "fatherRate"><font>%</font>
                     </dd>
              	</dl>
                  <dl>
                      <dt>商家佣金比例：</dt>
                      <dd>
                      <input type="text" class="c_scet" id = "ipt_sellerRate" name = "sellerRate"><font>%</font>
                     </dd>
              	</dl>
                  <dl>
                      <dt>业务员佣金比例：</dt>
                      <dd>
                      <input type="text" class="c_scet" id = "ipt_salesmanRate" name = "salesmanRate"><font>%</font>
                     </dd>
              	</dl>
              	<c:if test = "${productType.billType.ordinal() == '1' }">
	                <dl>
	                      <dt>佣金比例：</dt>
	                      <dd>
	                      <h6>
		                      <input type="text" class="c_text c_one" id = "ipt_userratedesc" name = "userRateDesc" value="会员佣金比例="
		                      ...
		                      <input class="inputtext2 inputtextn" name="used6"  id="tuserRateDesc" value="7/25" readonly="readonly"> 
	                      </h6>
	                      
	                      <h6>
	                      <input type="text" class="c_text c_one" id = "ipt_fatherratedesc" name = "fatherRateDesc" value="师父佣金补贴="
	                      ...
	                      <input class="inputtext2 inputtextn" name="used7"  id="tfatherRateDesc" value="7/25" readonly="readonly"> 
	                     </h6>
	                     </dd>
	              	</dl>
	             </c:if>
	             <c:if test = "${productType.billType.ordinal() == '2' }">
	                <dl>
	                      ...
	                      <dd>
	                      <h6>
		                      <input type="text" class="c_text c_one" id = "ipt_userratedesc" name = "userRateDesc" 
		                      onKeyDown="gbcount(this.form.userRateDesc,this.form.used6,22);"  onKeyUp="gbcount(this.form.userRateDesc,this.form.used6,22);" onblur = "gbcount(this.form.userRateDesc,this.form.used6,22);">
             ...
             <%--  </c:if> --%>
              ...
              <div class="compute">
                  <dl>
                      <dt>产品名称：</dt>
                      <dd><input type="text" class="c_text c_one" id = "ipt_productName" name = "productName"
                  ...
                  <dl>
                      <dt>贷款利率：</dt>
                      <c:if test = "${interestType != '1'}">
                      <dd>
                          <input type="text" class="c_scet c_cpt" id = "ipt_loanRate" name = "loanRate">
                          <font>%/月</font>
                          <samp>贷款页面利率：</samp>
                          <input type="text" class="c_scet c_cpt" id = "ipt_loanMinRate" name = "loanMinRate">
                          <font style="width:30px;">—</font>
                          <input type="text" class="c_scet c_cpt" id = "ipt_loanMaxRate" name = "loanMaxRate">
                          <font>%/月</font>
                     </dd>
                     </c:if>
                     <c:if test = "${interestType eq '1'}">
                      <dd>
                          <input type="text" class="c_scet c_cpt" id = "ipt_loanRate" name = "loanRate">
                          <font>‰/日</font>
                          <samp>贷款页面利率：</samp>
                    ...
                    <%--  <!-- 收益金 -->
                     <c:if test = "${interestType eq '2' || interestType eq '3'}">
                         <dd>
                              <input type="text" class="c_scet" id = "ipt_loanRate" name = "loanRate">
                              <font>%/月</font>   
                         </dd>
                     </c:if> --%>
              	</dl>
                  ...
                  <dl>
                      <dt>贷款金额：</dt>
                      <dd>
                      	<input type="text" class="c_scet c_cpt" id = "ipt_loanMinAmount" name = "loanMinAmount">
                          <font style="width:30px;">—</font>
                          <input type="text" class="c_scet c_cpt" id = "ipt_loanMaxAmount" name = "loanMaxAmount">
                          <font>万元</font>
                     </dd>
              	</dl>
              	<!-- 贷款期限 -->
              	<c:if test ="${interestType == '3' }">
              		<dl>
                        <dt>贷款期限：</dt>
                        <dd>
                        	<input type ="hidden" id = "ipt_loanMinTime" value = "6" name = "loanMinTime"/>
                        	<input type = "hidden" id = "ipt_loanMaxTime" value = "36" name = "loanMaxTime"/>
                         	<font>6</font>
                            <font style="width:30px;">—</font>
                            <font>36</font>
                            <font>个月</font>
              	...
              	<c:if test = "${interestType == '0' || interestType == '2'}">
                  <dl>
                      <dt>贷款期限：</dt>
                      <dd>
                      	<input type="text" class="c_scet c_cpt" id = "ipt_loanMinTime" name = "loanMinTime">
                          <font style="width:30px;">—</font>
                          <input type="text" class="c_scet c_cpt" id = "ipt_loanMaxTime" name = "loanMaxTime">
                          <font>个月</font>
                     </dd>
              	</dl>
              	</c:if>
                  ...
                  <dl>
                      <dt>还款方式：</dt>
                      <dd><input type="text" class="c_scet" id="ipt_repayment" name = "repayment" 
                      ...
                      <c:if test="${productType.billType.ordinal() != 0 }">
	                      ...
	                      <input type="text" class="c_scet" name = "brokerageSendDesc" placeholder="可不填" 
                  ...
                  <dl>
                      ...
                      <textarea  class="c_text m_height" id = "ipt_materials" name = "materials"
                       onKeyDown="gbcount(this.form.materials,this.form.used5,100);"  onKeyUp="gbcount(this.form.materials,this.form.used5,100);" onblur = "gbcount(this.form.materials,this.form.used5,100);"></textarea>
                  ...
                  <dl>
                      <dt>推荐首页：</dt>
                      <dd>
                          <span><input type="radio" name = "isDisplay"  id = "is_display" value = "1" class="c_rid">是</span>
                          <span><input type="radio" name = "isDisplay" id = "no_display"  value = "0" class="c_rid" checked>否</span>
                      </dd>
              	</dl>
                <dl>
                      <dt>产品排序：</dt>
                      <dd><input type="text" class="c_scet" id = "ipt_sortid" name = "sortid">&nbsp;&nbsp;排序值越大，显示越靠后</</dd>
              	</dl>
              	
              	<dl id = "div_isdisplay" class = "hidden">
                      <dt>首页排序：</dt>
                      <dd><input type="text" class="c_scet" id = "ipt_indexSortid" name = "indexSortid" >&nbsp;&nbsp;排序值越大，显示越靠后</</dd>
              	</dl>
                  <p>
